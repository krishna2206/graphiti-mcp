...
- Google Gemini API key (Graphiti uses Google Gemini for LLM inference, embeddings, and reranking)

> [!TIP]
> The simplest way to install FalkorDB is via Docker:

```bash
docker run -p 6379:6379 -p 3000:3000 -it --rm falkordb/falkordb:latest
```

Install Graphiti with FalkorDB and Google Gemini support:
...
> [!IMPORTANT]
> Graphiti uses Google Gemini for LLM inference, embeddings, and reranking. Ensure that a `GOOGLE_API_KEY` is set in your
> environment before using Graphiti.
...
You can use Docker Compose to quickly start FalkorDB:
...
The MCP server can be deployed using Docker with FalkorDB, making it easy to integrate Graphiti into your AI assistant
workflows.
...
Graphiti requires the following environment variables:

- `GOOGLE_API_KEY`: Your Google Gemini API key (required for LLM inference, embeddings, and reranking)

### Database Configuration

FalkorDB configuration can be customized by instantiating a database driver and passing it to the Graphiti constructor:

```python
from graphiti_core import Graphiti
from graphiti_core.driver.falkordb_driver import FalkorDriver

# Create a FalkorDB driver with custom configuration
driver = FalkorDriver(
    host="localhost",
    port=6379,
    username="falkor_user",  # Optional
    password="falkor_password",  # Optional
    database="my_custom_graph"  # Custom database name, defaults to "default_db"
)
...
The Gemini reranker uses the `gemini-flash-latest` model by default, which is optimized for
...
- **Configuration choices**:
    - LLM provider type (Google Gemini)
